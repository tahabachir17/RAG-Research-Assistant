...
class IngestionPipeline:
    """Discover papers, download PDFs, parse, clean, extract, and save JSON."""
    ...
    def run(self, query: str, max_results: int = 50) -> IngestionResult:
        ...
        for paper in papers:
            ...
            downloaded += int(download.downloaded)
            skipped += int(download.skipped)
            if download.error:
                failed += 1
                errors.append({"paper_id": download.paper_id, "stage": "download", "error": download.error})
                continue
            try:
                document = self.parser.extract(download.pdf_path, paper_id=paper.paper_id)
                ...
                self._write_processed_document(paper, metadata_record, document.to_dict(), cleaned.to_dict(), citations_to_dicts(citations))
                processed += 1
            except Exception as exc:
                failed += 1
                errors.append({"paper_id": paper.paper_id, "stage": "process", "error": str(exc)})
        ...
        return IngestionResult(query, self.discovery_provider, len(papers), downloaded, skipped, failed, processed, str(metadata_path), str(self.processed_dir), errors)

    def _write_processed_document(self, paper: Paper, metadata: dict[str, Any], raw_document: dict[str, Any], sectioned_document: dict[str, Any], citations: list[dict[str, Any]]) -> Path:
        output_dir = self.processed_dir / _safe_path_part(paper.primary_category or "unknown")
        output_dir.mkdir(parents=True, exist_ok=True)
        output_path = output_dir / f"{_safe_path_part(paper.paper_id)}.json"
        payload = {"paper_id": paper.paper_id, "metadata": metadata, "raw_document": raw_document, "sections": sectioned_document["sections"], "section_spans": sectioned_document.get("section_spans", {}), "citations": citations}
        output_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
        return output_path


def run_ingestion(query: str, max_results: int = 50, data_dir: str | Path = "data", discovery_provider: str = "auto") -> IngestionResult:
    return IngestionPipeline(data_dir=data_dir, discovery_provider=discovery_provider).run(query=query, max_results=max_results)


def _discovery_metadata(paper: Paper) -> dict[str, Any]:
    reserved = set(Paper.__dataclass_fields__)
    return {key: value for key, value in paper.to_dict().items() if key not in reserved and value not in (None, "", [])}
...
def main() -> None:
    ...
    parser.add_argument("--max-results", type=int, default=50, help="Maximum papers to ingest")
    parser.add_argument("--data-dir", default="data", help="Project data directory")
    parser.add_argument("--discovery-provider", choices=("auto", "feyman", "arxiv", "alphaxiv-mcp", "research-apis"), default="auto", help="Discovery provider. Default auto uses alphaXiv MCP, then falls back to arXiv.")
    args = parser.parse_args()
    result = run_ingestion(args.query, max_results=args.max_results, data_dir=args.data_dir, discovery_provider=args.discovery_provider)
