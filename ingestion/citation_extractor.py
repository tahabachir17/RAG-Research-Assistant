...
def extract_citations(sectioned_doc_or_text: Any) -> list[Citation]:
    ...
    return [_parse_reference(index, entry) for index, entry in enumerate(entries, start=1)]
...
def _guess_title(raw_text: str) -> str | None:
    ...
    parts = [part.strip() for part in re.split(r"\.\s+", without_marker) if part.strip()]
    if len(parts) >= 2:
        candidate = parts[1]
