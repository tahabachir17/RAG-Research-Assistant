...
SECTION_PATTERNS: dict[str, re.Pattern[str]] = {
    ...
    "related_work": re.compile(r"^(?:\d+\.?\s*)?(?:related work|background|prior work)$", re.I),
    "method": re.compile(
    ...
    "conclusion": re.compile(r"^(?:\d+\.?\s*)?(?:conclusions?|discussion and conclusion)$", re.I),
...
def extract_sections(pages: Iterable[PageLike | Mapping[str, object]], paper_id: str) -> SectionedDoc:
    ...
    cleaned_sections = {key: _join_section_lines(value) for key, value in sections.items()}
    return SectionedDoc(paper_id=paper_id, sections=cleaned_sections, section_spans=spans)
...
def _page_number(page: PageLike | Mapping[str, object], fallback: int) -> int:
    value = page.get("page_number", fallback) if isinstance(page, Mapping) else getattr(page, "page_number", fallback)
    try:
        return int(value)
