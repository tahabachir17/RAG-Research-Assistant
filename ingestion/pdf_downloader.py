...
class PDFDownloader:
    """Download ArXiv PDFs into data/raw/arxiv/{category}/{paper_id}.pdf."""
    ...
    def download(self, paper: Paper | dict[str, Any]) -> DownloadResult:
        ...
        if pdf_path.exists() and not self.overwrite:
            return DownloadResult(
                paper_id=paper_id, pdf_path=pdf_path, downloaded=False, skipped=True
            )
        ...
        try:
            with httpx.stream(
                ...
                if "pdf" not in content_type and not urlparse(pdf_url).path.endswith(
                    ".pdf"
                ):
                    return DownloadResult(
                        paper_id=paper_id,
