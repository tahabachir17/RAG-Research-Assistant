        ...
        pages=[
            RawPage(
                ...
                text=(
                    ...
                    "References\n[1] Lewis et al. Retrieval-Augmented Generation. 2020. arXiv:2005.11401"
                ),
                char_count=190,
            )
        ],
...
def test_extract_citations_from_references_section():
    class Doc:
        sections = {
            "references": "[1] Lewis et al. Retrieval-Augmented Generation. 2020. arXiv:2005.11401 "
            "[2] Vaswani et al. Attention Is All You Need. 2017. doi:10.5555/3295222.3295349"
        }
...
def test_research_api_provider_can_be_built():
    discovery = build_discovery("research-apis")

    assert isinstance(discovery, ResearchAPIPaperDiscovery)


def test_dedupe_papers_uses_arxiv_id_doi_and_title():
    papers = [
        _paper("2005.11401v1", "Retrieval Augmented Generation", "RAG", doi="10.1/a"),
        _paper("2005.11401v2", "Retrieval Augmented Generation", "RAG"),
        _paper("9999.00001", "Different title", "Different", doi="10.1/a"),
        _paper("9999.00002", "Retrieval   augmented generation!", "Same title"),
    ]
...
def test_score_papers_prefers_relevant_title_and_abstract_without_embeddings():
    papers = [
        _paper("1", "Graph neural networks", "Molecular property prediction"),
        _paper("2", "Retrieval augmented generation", "RAG retrieves passages for question answering"),
    ]

    scored = score_papers("retrieval augmented generation", papers, use_embeddings=False)
...
def test_alphaxiv_provider_can_be_built():
    discovery = build_discovery("alphaxiv-mcp")

    assert isinstance(discovery, AlphaXivMCPPaperDiscovery)


def test_alphaxiv_argument_sets_preserve_domain_phrases():
    argument_sets = _alphaxiv_argument_sets(
        "earthquake analysis using deep learning",
        difficulty=5,
        groq_api_key=None,
    )
    ...
    def search(self, query: str, max_results: int = 50):
        return [
            _paper(
                ...
                "RAG combines parametric and retrieved knowledge.",
            )
        ]


def test_auto_discovery_falls_back_to_arxiv_when_alphaxiv_fails():
    discovery = AlphaXivThenArxivDiscovery(
        primary=BrokenDiscovery(), fallback=FakeArxivFallback()
    )
...
def test_default_discovery_uses_auto_fallback_provider():
    ...
    assert isinstance(discovery, AlphaXivThenArxivDiscovery)
