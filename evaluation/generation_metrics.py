...
def qualifying_item_precision(
    ...
    if not expected_set:
        return None
    return (
        len(predicted_set & expected_set) / len(predicted_set) if predicted_set else 0.0
    )


def qualifying_item_recall(
    predicted: Iterable[str], expected: Iterable[str]
) -> float | None:
    predicted_set, expected_set = _normalized_set(predicted), _normalized_set(expected)
    return (
        len(predicted_set & expected_set) / len(expected_set) if expected_set else None
    )
...
def claim_level_citation_coverage(claims: Sequence[Mapping[str, Any] | bool]) -> float:
    values = [
        bool(claim if isinstance(claim, bool) else claim.get("cited", False))
        for claim in claims
    ]
    return _boolean_rate(values)


def required_field_completeness(
    present: Iterable[str], required: Iterable[str]
) -> float:
...
def truncation_rate(finish_reasons: Iterable[str | None]) -> float:
    ...
    if not reasons:
        return 0.0
    return sum(
        bool(reason and str(reason).casefold() in truncated) for reason in reasons
    ) / len(reasons)
...
def unsupported_claim_rate(verdicts: Iterable[str]) -> float | None:
    ...
    return (
        sum(label == "unsupported" for label in judged) / len(judged)
        if judged
        else None
    )
...
def direct_context_precision(
    retrieved: Iterable[str], reference: Iterable[str], *, reviewed: bool = True
) -> float | None:
    """Exact chunk-ID precision, unavailable when gold evidence was not reviewed."""

    if not reviewed:
        return None
    retrieved_set, reference_set = set(retrieved), set(reference)
...
def direct_context_recall(
    retrieved: Iterable[str], reference: Iterable[str], *, reviewed: bool = True
) -> float | None:
    """Exact chunk-ID recall, unavailable when gold evidence was not reviewed."""
